...
class DevolucaoController extends Controller
{
		...
		if ($request->hasFile('file')){
			...
			//if($this->validaChave($xml->NFe->infNFe->attributes()->Id)){
				...
				foreach($xml->NFe->infNFe->det as $item) {
					$prod = Produto::where('nome', $item->prod->xProd)->first();
					$item = [
						...
						'cst_csosn' => $prod->CST_CSOSN, //$config->CST_CSOSN_padrao,
						'cst_pis' => $prod->CST_PIS, //$config->CST_PIS_padrao,
						'cst_cofins' => $prod->CST_COFINS, //$config->CST_COFINS_padrao,
						'cst_ipi' => $prod->CST_IPI, //$config->CST_IPI_padrao,
						'perc_icms' => $prod->perc_icms, //$tributacao->icms,
						'perc_pis' => $prod->perc_pis, //$tributacao->pis,
						'perc_cofins' => $prod->perc_cofins, //$tributacao->cofins,
						'perc_ipi' => $prod->perc_ipi, //$tributacao->ipi
					];
	...
	public function salvar(Request $request){
		...
		foreach($data['itens'] as $i){
			$prod = Produto::where('nome', $i['xProd'])
			->orWhere('referencia', $i['codigo'])
			->orWhere('codBarras', $i['codBarras'])->first();;
	...
	public function enviarSefaz(Request $request){
		...
		if($devolucao->estado == 0 || $devolucao->estado == 2){
			...
			if(substr($resultado, 0, 4) != 'Erro'){
				...
				$devolucao->serie = $config->numero_serie_nfe;
				$devolucao->save();
			}else{
				$devolucao->estado = 2;
				$devolucao->save();
