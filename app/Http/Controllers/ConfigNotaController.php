...
class ConfigNotaController extends Controller
{
	...
	public function save(Request $request){
		...
		if($request->hasFile('file')){
			...
			$nomeImagem = "logo".$this->empresa_id.".".$extensao;
		...
		if($request->id == 0){
			...
			$config->CFOP_saida_estadual = $request->CFOP_saida_estadual;
			$config->CFOP_saida_inter_estadual = $request->CFOP_saida_inter_estadual;
