	...
	public function gerarDevolucao($devolucao){
		...
		foreach($devolucao->itens as $i){
			...
			if($config->UF != $devolucao->fornecedor->cidade->uf){
				if($i->cfop == '6656' || $i->cfop == '6655' || $i->cfop == '6652'){
					$CFOP = '6661';	
				}elseif($i->cfop == '6102' || $i->cfop == '6101'){ 
					$CFOP = '6202';			
				}elseif($i->cfop == '6404' || $i->cfop == '6401'){ 
					$CFOP = '6411';			
				}elseif($i->cfop == '6920'){ 
					$CFOP = '6921';	
				//}elseif($i->cfop == '6908'){ 
				...
				if($i->cfop == '5656' || $i->cfop == '5655' || $i->cfop == '5652'){
					$CFOP = '5661';	
				}elseif($i->cfop == '5102' || $i->cfop == '5101'){ 
					$CFOP = '5202';			
				}elseif($i->cfop == '5405' || $i->cfop == '5401'){ 
					$CFOP = '5411';			
				}elseif($i->cfop == '5920'){ 
					$CFOP = '5921';	
				//}elseif($i->cfop == '5908'){ 
			...
			if($CFOP == '5661' || $CFOP == '6661'){
				$stdProd->uTrib = 'KG';
				$stdProd->qTrib = $i->produto->peso_liquido * $i->quantidade;
				$stdProd->vUnTrib = round($this->format($i->valor_unit) / $i->produto->peso_liquido, 10);				
			}else{
				$stdProd->uTrib = $i->unidade_medida;
				$stdProd->qTrib = $i->quantidade;
				$stdProd->vUnTrib = $this->format($i->valor_unit);
...
*/
				...
				if($CFOP == '5661' || $CFOP == '6661' || $i->cst_csosn == '41'){ 
					$stdICMSST = new \stdClass();
					$stdICMSST->item = $itemCont;
					$stdICMSST->orig = 0;
					$stdICMSST->CST = $i->cst_csosn;
				...
				if($i->cst_csosn == '00' || $i->cst_csosn == '102'){
					$CSOSN = '900';
				}else{
					$CSOSN = $i->cst_csosn; 
				}	
		...
		//TAG ANP
			...
			if($CFOP == "5661" || $CFOP == "6661"){
				...
				$stdComb->cProdANP = '210203001';
				$stdComb->descANP = 'GLP'; 
				$stdComb->UFCons = $devolucao->fornecedor->cidade->uf;

                $stdComb->pGLP = 100;
                $stdComb->pGNn = 0;
                $stdComb->pGNi = 0;
                $stdComb->vPart = $this->format($i->valor_unit);
		...
		$stdVol->esp = '';
		$stdVol->nVol = '';
		$stdVol->pesoL = $somaPesoL;
		$stdVol->pesoB = $somaPesoB;
